...
- **Hedera**: deprioritized by explicit user decision ("we can think of Hedera later"). Research already done if picked back up: Circle Gateway has zero Hedera support (confirmed against Circle's own supported-chains doc); USDC on Hedera is a native Hedera Token Service asset, not a plain ERC-20 (needs "association" before an account can hold it — a real structural difference, not just a config swap); thirdweb has no built-in Hedera chain definition (would need a manual `defineChain`, chain IDs: mainnet 295 / testnet 296) but does have a Blade Wallet connector stub. Minimal realistic path if resumed: a fully separate direct-payment flow (HBAR or associated USDC, verified via Hedera's mirror-node REST API), independent of the existing Arc/Gateway code — not a drop-in extension of it.
