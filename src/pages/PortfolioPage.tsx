...
export function PortfolioPage() {
    ...
    return <section className="repository-page portfolio-page">
        <header className="portfolio-header"><h1>My Portfolio</h1><a className="btn btn-outline" href="/works">My Listings ↗</a></header>
        ...
        <div className="portfolio-toolbar">
            ...
            <button className="btn btn-outline" disabled={busy} onClick={signIn}>{busy?'Please wait…':history?.wallet?'Switch / verify wallet':'Connect and verify wallet'}</button>
        </div>
        ...
        {!history ? <p role="status">Loading portfolio…</p> : !rows.length ? <p className="hint">No {tab} recorded yet. History starts with purchases made after this feature was added.</p> : <div className="portfolio-table-wrap"><table><thead><tr><th>Repository</th><th>Amount</th><th>Payment</th><th>Date</th><th>{tab==='sales'?'Buyer':'Access'}</th></tr></thead><tbody>{rows.map(p=><tr key={p.id}>
            <td><a href={`/${p.repoFullName}`}>{p.repoFullName}</a></td><td>{p.amount} {p.currency}</td><td>{p.channel==='x402'?'x402':p.checkoutContract?'Margit checkout':'Wallet'}<br/><small>{p.status==='confirmed'?'Onchain confirmed':'Gateway accepted'}</small>{p.onchainPurchaseId && <small>{history.graph?.ids.includes(p.id) ? 'Indexed by The Graph' : history.graph?.configured ? history.graph.available ? 'Awaiting indexer' : 'Indexer temporarily unavailable' : 'Graph indexing not configured'}</small>}{p.transactionHash && <a href={`${arcTestnet.blockExplorers?.[0]?.url}/tx/${p.transactionHash}`} target="_blank" rel="noreferrer"> ↗</a>}</td><td>{new Date(p.createdAt).toLocaleDateString()}</td>
