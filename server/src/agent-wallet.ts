...
export async function resolveAgentWallet(session = 'internal', previewMode?: 'shared' | 'personal' | 'circle') {
...
export async function selectAgentWallet(session: string, mode: string) {
    if (mode !== 'shared' && mode !== 'personal' && mode !== 'circle') throw new Error('Choose demo, personal, or Circle wallet.');
...
export async function depositAgentGateway(session: string, amount: string, requestId: string) {
    ...
    const address = selected.mode === 'circle' ? selected.address : privateKeyToAccount(privateKey!).address;
