...
const reportPath='contracts/smoke.arc-testnet.json';
...
const approval=await wallet.writeContract({address:order.token,abi:tokenAbi,functionName:'approve',args:[metadata.address,order.amount],gas:100000n,maxFeePerGas,maxPriorityFeePerGas:0n});
if((await client.waitForTransactionReceipt({hash:approval})).status!=='success')throw new Error('Approval failed');
const hash=await wallet.writeContract({address:metadata.address,abi:checkoutAbi,functionName:'buy',args:[order,Number(signature.v),signature.r,signature.s],gas:200000n,maxFeePerGas,maxPriorityFeePerGas:0n});
fs.writeFileSync('contracts/smoke.pending.local',hash);
